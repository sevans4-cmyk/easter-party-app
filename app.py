...
tab_signup, tab_attendance, tab_food, tab_manage = st.tabs(["📝 Sign Up", "👥 Who's Coming", "🍽️ Potluck Food", "🔧 Manage My Signup"])
...
with tab_attendance:
    ...
    if len(df) == 0:
        ...
        attendance_df = df[["Name", "Attending", "Attendees", "Notes"]].copy()
...
with tab_manage:
    st.write("### 🔧 Edit or Delete Your Signup")
    edit_code_input = st.text_input("Your Edit Code (e.g. A1B2C3)", placeholder="A1B2C3")
    
    if st.button("🔍 Load My Signup"):
        matches = df[df["Edit Code"] == edit_code_input.upper()]
    ...
    if "edit_row" in st.session_state:
        ...
        with st.form("edit_form"):
            ...
            attending = st.selectbox("Are you coming?", ["Yes", "Maybe", "No"], 
                                   index=["Yes", "Maybe", "No"].index(st.session_state.edit_row["Attending"]))
            attendees = st.text_area("Who is attending? (one per line)", 
                                   value=st.session_state.edit_row["Attendees"], height=100)
            food_item = st.text_input("What are you bringing?", value=st.session_state.edit_row.get("Food Item", ""))
            category = st.selectbox("Category", ["Apps", "Side Dish", "Main", "Dessert", "Drinks"], 
                                  index=["Apps", "Side Dish", "Main", "Dessert", "Drinks"].index(st.session_state.edit_row.get("Category", "Apps")))
            notes = st.text_area("Notes", value=st.session_state.edit_row.get("Notes", ""))
            
            col1, col2 = st.columns(2)
            with col1:
                update_submitted = st.form_submit_button("💾 Update Signup")
            with col2:
                delete_submitted = st.form_submit_button("🗑️ Delete Signup", type="primary")
            
            if update_submitted:
                df.loc[st.session_state.edit_index, ["Name", "Attending", "Attendees", "Food Item", "Category", "Notes"]] = \
                    [name, attending, attendees, food_item, category, notes]
                df.to_csv(CSV_FILE, index=False)
                st.success("✅ Signup updated!")
